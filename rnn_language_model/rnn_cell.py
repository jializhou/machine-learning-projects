...
class BasicLSTMCell(RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
      ...
      #new_c = (c * sigmoid(f + self._forget_bias) + self._activation(j)) #no input gate
      new_c = (c * sigmoid(1.0) + sigmoid(i) *self._activation(j))  # no forget gate    
      new_h = self._activation(new_c) # no output gate 
      # new_h = self._activation(new_c)* sigmoid(o) #lstm
      if self._state_is_tuple:
